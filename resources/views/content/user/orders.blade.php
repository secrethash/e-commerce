...
@section('content')
    ...
    <div class="card">
      <img class="card-img-top" src="{{asset('frontend/assets/images/banner-image/14.jpg')}}" alt="Title">
      <div class="card-body p-4">
        <h1 class="card-title text-center my-4">Order History</h1>
        <div class="card-text">
            <div class="row g-3">
                @forelse ($orders as $order)
                    @if ($order->items->count() >= 1)
                        ...
                        <div class="col-12 col-md-3">
                            <div class="card px-0">
                                <img class="card-img-top" src="{{$image}}" alt="{{$firstItem?->name}}">
                                <div class="card-body px-0 pb-0">
                                    <h6 class="card-title placeholder-glow px-3">
                                        @if ($firstItem)
                                            {{str($firstItem->name)->words(3, '...')}}
                                            @if($order->items->count() > 1)
                                                {{'+ ' . $order->items->count() - 1}}
                                            @endif
                                        @else
                                    ...
                                    <strong class="card-subtitle px-3 text-danger fw-light">
                                        <x-heroicon-o-hashtag width="18" />
                                        {{$order->number}}
                                    </strong>
                                    <p class="card-text mt-4 mb-0">
                                        <ul class="list-group rounded-0">
                                            <li class="list-group-item bg-danger text-light text-uppercase">Items &mdash;</li>
                                            @foreach ($order->items as $item)
                                                @if ($loop->index === $maxItems)
                                                    <li class="list-group-item fw-bolder">
                                                        @php
                                                            $orderItemsCount = $order->items->count() - $maxItems;
                                                        @endphp
                                                        {{"+" . $orderItemsCount}} {{'More '. str('Items')->plural($orderItemsCount)}}
                                                ...
                                                @else
                                                    <li class="list-group-item d-flex justify-content-between align-items-start">
                                                        <div class="ms-2 me-auto">
                                                            <div class="fw-bold">{{str($firstItem->name)->words(4, '...')}}</div>
                                                            <small class="text-muted"><strong>SKU:</strong> {{$item->sku}}</small>
                                                        </div>
                        ...
                        <div class="row g-4 card-body p-md-5 align-items-center justify-content-center">
                            ...
                            <div class="col-12 d-flex align-items-center justify-content-center mb-1 card-text">
                                <div class="flex-column text-center">
                                    <div class="mb-3">
                                        <h2>No <span class="fw-light">Orders!</span></h2>
                                    </div>
                                    <div class="mb-3">
                                        <p>No Orders in your account yet. Order new products by visiting our shop.</p>
                                    </div>
                                    <div class="mb-3">
